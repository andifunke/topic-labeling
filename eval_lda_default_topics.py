...
from utils import init_logging, load, log_args, TopicsLoader, tprint
import warnings
...
def parse_args():
    ...
    parser.add_argument("--cores", type=int, required=False, default=8)
...
def main():
    ...
    file = '/media/andreas/Sounds/workspace/master_cloud/corpora/preprocessed/LDAmodel/noun/bow/topics/'\
           'OnlineParticipation_topics_ep20.csv'
           # 'PoliticalSpeeches_topics_ep20.csv'
    topics = pd.read_csv(file, index_col=[0, 1, 2, 3, 4], header=0)
    tprint(topics)
    ...
    if use_coherence:
        ...
        df = eval_coherence(
            ...
            keyed_vectors=None, metrics=['c_npmi'], window_size=None,
    ...
    file = file.replace('_topics_ep20.csv', '_scores_ep20.csv')
    if exists(file):
        file = file.replace('.csv', '_' + str(time()).split('.')[0] + '.csv')
