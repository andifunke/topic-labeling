...
It is the script to generate candidate labels, unsupervised best labels and labels from SVM ranker
supervised model. Update the parameters in this script. Also after you download the files mentioned in
readme and if you keep them in different path change it over here.

Parameters: -cg To generate candiidate labels. -us To get unsupervised labels. -s To get supervised
labels. Ideally should first use -cg to get candiate label file before going for unsupervised or
supervised model. But can be used directly if you already have your candidate label file for the
topics. Example for topics given in toy_data/toytopics.csv """
...
# Unsupevised model parameters Number of unsupervised labels needed (In general should be less than
# candidate labels, till you have your own file and then depends on number of labels)
num_unsup_labels = 3
# The file which contains candiate generation output. Also used to get supervised output
cand_gen_output = "output_candidates"
...
pagerank_model = "support_files/en/pagerank-titles-sorted.txt"  # This is precomputed pagerank model
# needed to genrate pagerank features.
svm_classify = "support_files/en/svm_rank_classify"  # SVM rank classify. After you download SVM Ranker
# classify gibve the path of svm_rank_classify here
pretrained_svm_model = "support_files/en/svm_model"  # This is trained supervised model on the whole our
# dataset. Run train train_svm_model.py if you want a new model on different dataset.
...
if args.candidates:  # It calls unsupervised_labels python file to get labels in unsupervised way
    query1 = "python cand_generation.py " + str(
        num_candidates) + " " + doc2vecmodel + " " + word2vecmodel + " " + data + " " + output_filename \
             + " " + doc2vec_indices_file + " " + word2vec_indices_file
    print(query1)
...
if args.supervised:  # It calls supervised_labels python file to get labels in supervised way.
    query3 = "python supervised_labels.py " + str(
        num_sup_labels) + " " + pagerank_model + " " + data + " " + cand_gen_output + " " \
             + svm_classify + " " + pretrained_svm_model + " " + out_sup
