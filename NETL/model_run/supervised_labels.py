...
parser.add_argument("svm_classify")
# path to the SVM Ranker classify binary file. Needs to be downloaded from the path provided in Readme.
parser.add_argument("trained_svm_model")
...
def get_lt_ranks(lab_list, num):
    ...
    for item in lab_list:
        trigrams = [item[i:i + 3] for i in
                    range(0, len(item) - 2)]  # get the trigrams for label candidate.
...
dataframe namely Pagerank, letter trigram, Topic overlap and Number of words in a label. Additionally 
DatFrame will also be given the label name, topic_id and an avg_val which is average annotator value. 
It is just given a value of 3 here but can be anything as it does not make a difference in prediction. 
Only important when we have to train SVM model. """
...
def prepare_features(letter_tg_dict, page_rank_dict, cols, feature_names):
    ...
    for x in range(0, len(letter_tg_dict)):
        ...
        for k in a:
            ...
            com_word_length = len(
                set(word_labels).intersection(set(topic_list[x])))  # Extracting topic overlap.
            ...
            new_list.append(3)
            # This could be just any value appended for the sake of giving a column for annotator rating
            # needed in SVM Ranker classify
            temp = pd.Series(new_list, index=cols)
            temp_frame = temp_frame.append(temp, ignore_index=True)
            temp_frame = temp_frame.fillna(0)
            # Just filling in case a label does not have a pagerank value. Generally should not happen
...
cols = ['label', 'topic_id', 'letter_trigram', 'prank', 'lab_length', 'common_words',
        'avg_val']  # Name of columns for DataFrame.
...
def convert_dataset(test_file, feature_names):
    ...
    for i in range(len(test_file)):
        ...
        mystring = str(test_file[i:i + 1]["avg_val"].values[0]) + " " + "qid:" + str(
            int(test_file[i:i + 1]["topic_id"].values[0]))
...
def get_predictions(test_set, num):
    ...
    # deleting the ttest file and prediction file generated as part of code to run svm_classify
    os.system(query3)
