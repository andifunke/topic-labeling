...
Also after you download the files mentioned in readme and if you keep them in different path change
it over here.
...
Ideally should first use -cg to get candiate label file before going for unsupervised or supervised
model.
...
# Common Parameters
# data = "toy_data/toytopics.csv"  # The file in csv format which contains the topic terms that needs
# a label.
data = "data/toy_data/toytopics.csv"
...
num_unsup_labels = 3  # Number of unsupervised labels needed (In general should be less than candidate
#  labels, till you have your own file and then depends on number of labels)
cand_gen_output = "output_candidates"  # The file which contains candiate generation output. Also used
# to get supervised output
...
if args.candidates:  # It calls unsupervised_labels python file to get labels in unsupervised way
    query1 = "python cand_generation_gen32.py " + str(
...
if args.supervised:  # It calls supervised_labels python file to get labels in supervised way.
    query3 = "python supervised_labels.py " + str(
        num_sup_labels) + " " + pagerank_model + " " + data + " " + cand_gen_output + " " + svm_classify \
             + " " + pretrained_svm_model + " " + out_sup
