...
def get_labels(topic, nb_labels, d2v, w2v, w2v_indexed, d_indices, w_indices):
    ...
    valword2vec = 0.0
    store_indices = []
    print("Processing Topic number " + str(topic.name))
    topic = list(topic)
    topic_len = len(topic)
    print(topic)
...
def load_embeddings(d2v_path, w2v_path):
    ...
    # w2v.delete_temporary_training_data(replace_word_vectors_with_normalized=False)
...
def index_embeddings(d2v, w2v, d2v_indices, w2v_indices):
    ...
    """
    print(len(d2v_indices), sorted(d2v_indices)[:10])
    print(len(w2v_indices), sorted(w2v_indices)[:10])
    ...
    print(len(d2v.wv.syn0norm))
    print(len(d2v.docvecs.vectors_docs_norm))
    print(len(w2v.wv.syn0norm))
    print(len(w2v_indexed))

    print(d2v.wv.syn0norm)
    print(d2v.docvecs.vectors_docs_norm)
    print(w2v.wv.syn0norm)
    print(w2v_indexed)
    return w2v_indexed
...
def load_topics(topics_path, metrics, params, nbtopics, print_sample=False):
    ...
    topics = pd.read_csv(topics_path, index_col=None)[:1]
...
def main():
    ...
    topics = load_topics(
        topics_path=topics_file, metrics=metrics, params=params, nbtopics=nb_topics, print_sample=True
    )
