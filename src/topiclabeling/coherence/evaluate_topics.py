...
def eval_coherence(
    ...
    for metric in metrics:
        ...
        if metric == "u_mass":
            ...
            with open(texts) as fp:
                for size, _ in enumerate(fp, 1):
                    pass
            fp = open(texts, "r")
            logg(f"Loading texts from {texts}")
            txt = tqdm(
                map(lambda x: x.strip().split(), fp),
                total=size,
                unit=" documents",
            )
        else:
...
def main():
    ...
    if args.use_w2v:
        ...
        ps2 = unique_topics.apply(
            pairwise_similarity, kvs=kvs, ignore_oov=False, axis=1
        )
        df_sims = pd.concat(
            {
                "mean_similarity": ms,
    ...
    if args.d2v:
        ...
        df = eval_coherence(
            ...
            corpus=None,
            texts=None,
            keyed_vectors=d2v.wv,
            metrics=[],
            window_size=None,
            suffix="_d2v.wv.custom",
            cores=args.cores,
            topn=args.topn,
        )
        gc.collect()
