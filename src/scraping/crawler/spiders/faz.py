...
from scraping.crawler.items import CrawlerItem
...
class FazSpider(CrawlSpider):
    ...
    def __init__(self, categories="politik|wirtschaft", *args, **kwargs):
        ...
        FazSpider.rules = (
            Rule(
                LinkExtractor(
                    allow=(
                        r"aktuell\/(" + categories + r")\/.*\/$",
                        r"aktuell\/(" + categories + r")\/.*\/s\d+\.html",
                    ),
                ),
                follow=True,
            ),
            Rule(
                LinkExtractor(
                    allow=(r"aktuell\/(" + categories + r").+\.html"),
                ),
                callback="parse_page",
            ),
        )
    ...
    def parse_page(self, response):
        ...
        item["keywords"] = [
            s for s in response.xpath('//meta[@name="keywords"]/@content').extract()
        ]
        item["published"] = response.css(
            ".atc-MetaTime::attr(datetime)"
        ).extract_first()
        ...
        item["author"] = [
            s
            for s in response.selector.xpath(
