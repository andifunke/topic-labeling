...
from topiclabeling.constants import DATA_BASE, DATASETS_FULL
from topiclabeling.rank_labels_train_svm import (
    load_topics,
    load_labels,
    load_pageranks,
    generate_lt_feature,
...
output_path = join(
    svm_path,
    f"supervised_labels_{ratings_version}__testset{testsets}__trainset{trainsets}",
)
...
def predict(
    test_set, pred_chunk_size, tmp_file, svm_classifier_file, svm_model_file, out_file
):
    ...
    query = " ".join([svm_classifier_file, tmp_file, svm_model_file, "predictionstemp"])
    print(query)
    print()
    os.system(query)
...
def main():
    ...
    predict(
        test_list, nb_labels, tmp_file_path, svm_classify_path, svm_model, output_path
    )
