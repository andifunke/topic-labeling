...
public final class WikipediaPagerank {
	...
	private static final File PAGE_ID_TITLE_SQL_FILE = new File("dewiki-latest-page.sql.gz");           // Original input file
	private static final File PAGE_ID_TITLE_RAW_FILE = new File("wikipedia-pagerank-page-id-title.raw");  // Cache after preprocessing
	
	private static final File PAGE_LINKS_SQL_FILE = new File("dewiki-latest-pagelinks.sql.gz");   // Original input file
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < 1000; i++) {
			...
			pageranks = pr.pageranks;
			printPagerankChangeRatios(prevPageranks, pageranks);
			printTopPages(pageranks, idToTitle, 30);
			prevPageranks = pageranks.clone();
		}

		// printTopPages(pageranks, idToTitle, pageranks.length);
	...
	private static void printTopPages(double[] pageranks, Map<Integer,String> titleById, int num_pages) {
