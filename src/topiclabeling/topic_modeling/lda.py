...
from topiclabeling.utils.constants import DATASETS, PARAMS, LDA_DIR, NB_TOPICS
from topiclabeling.utils.utils import init_logging, log_args, load
...
class EpochLogger(Metric):
    ...
    def get_value(self, **kwargs):
        if self.logg is not None:
            self.logg(
                f"--- {self.title} --- Epoch #{self.epoch:02d} --- {self.message} ---"
            )
        self.epoch += 1
        gc.collect()
...
class DiffMetric(Metric):
    ...
    def diff(
        ...
        fst_topics = [
            {w for (w, _) in self.model.show_topic(topic, topn=num_words)}
...
def init_callbacks(
    ...
    return [
        ...
        EpochLogger(
            title=dataset,
            message=f"[{version}, {param}, {nbtopics}, {tfidf}]" f" epoch finished",
            logg=LOGG,
        ),
    ]
...
def parse_args():
    ...
    parser.add_argument(
        "--callbacks", dest="use_callbacks", action="store_true", required=False
    )
    parser.add_argument(
        "--no-callbacks", dest="use_callbacks", action="store_false", required=False
    )
...
def main():
    ...
    for param in params:
        ...
        for nbtopics in nbs_topics:
            ...
            model_path = join(
                model_dir, f"{dataset}_LDAmodel_{param}{_split}_{nbtopics}_ep{epochs}"
            )
