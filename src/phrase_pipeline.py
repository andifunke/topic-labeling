...
def get_removable_tokens(df_in):
    ...
    df_out = pd.DataFrame.from_records(
        remove_token, columns=[SENT_IDX, TOK_IDX]
...
def preprocess_dewac(df):
    ...
    df = (
        df.groupby(HASH, sort=False, as_index=False)
...
def process_subset(df):
    ...
    df_ent = (
        ...
        .groupby(ENT_IDX, as_index=False)
        .agg(concat_entities)  # concatenate entities
        .assign(
        ...
        .astype(
            {  # set annoation columns as categorical for memory savings
                POS: "category",
                ENT_IOB: "category",
                ENT_TYPE: "category",
            }
    ...
    df_np = (
        ...
        .groupby(NOUN_PHRASE, as_index=False)
        .agg(concat_entities)
        .assign(
            length=lambda x: x.tok_idx.apply(lambda y: len(y)),
            POS="NPHRASE",
        ...
        .astype({POS: "category", ENT_IOB: "category", ENT_TYPE: "category"})
    )
    logg("collect: %d" % gc.collect())
    logg(memstr())
    ...
    df_loc = (
        df.loc[(df[ENT_IDX] > 0) & (df.POS != SPACE)]
        .groupby(ENT_IDX, as_index=False)
        .agg(aggregate_streets)
        .query("token != False")
        .assign(
            length=lambda x: x.tok_idx.apply(lambda y: len(y)),
    ...
    df_glued = df_glued.loc[
        df_glued.POS != "SPACE", [HASH, POS, SENT_IDX, TOK_IDX, TOKEN]
    ].astype(
        {
            HASH: np.int64,
            POS: "category",
...
def main(corpus, batch_size=None):
    ...
    for i, grp in enumerate(df_main, last_cnt):
        ...
        if (batch_size is not None and (i % batch_size == 0)) or (i == length):
            ...
            t_b = int(time() - t_a)
            logg(
                "subset done in {:02d}:{:02d}:{:02d}".format(
                    t_b // 3600, (t_b // 60) % 60, t_b % 60
                )
            )
...
if __name__ == "__main__":
    ...
    t_1 = int(time() - t_0)
    logg(
        "all done in {:02d}:{:02d}:{:02d}".format(
            t_1 // 3600, (t_1 // 60) % 60, t_1 % 60
        )
    )
